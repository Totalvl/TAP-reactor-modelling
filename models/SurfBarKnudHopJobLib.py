...
#     for i in range(0,Nr+1):
    ...
    for i in range(Nx):
        ...
        while erCp > err*np.max(Cp_n) or erTheta > err*np.max(thetaIntIn):
            ...
            b2[i, -1] = b2[i, -1] + 2*beta*dr*k_ent*Cs[i]  # boundary conditions
            diags[i,:] = 1 + 2*beta + k1*(1-thetaInt_1[i])
            diags[i, 0] = 1 + 2*beta + k1*(1-thetaInt_1[i,0]) + 2*beta*dr*k_ext
...
# @nb.jit()
def MicroPDiff_linalg(CpIn, thetaIntIn, R, ASpore, dt, dr, Cs, eps_p, pars_int):
    """
    Vectorized implementation of solver_BE_simple using also
    a sparse (tridiagonal) matrix for efficiency.
...
#     for i in range(0,Nr+1):
    ...
    for i in range(Nx):
        ...
        while erCp > err*np.max(Cp_n) or erTheta > err*np.max(thetaIntIn):
            ...
            b2[i, -1] = b2[i, -1] + 2*beta*dr*k_ent*Cs[i]  # boundary conditions
            diags[i,:] = 1 + 2*beta + k1*(1-thetaInt_1[i])
            diags[i, -1] = 1 + 2*beta + k1*(1-thetaInt_1[i,0]) + 2*beta*dr*k_ext
...
# @nb.jit()
def MicroPDiff_multiprocess(num, CpIn, thetaIntIn, R, ASpore, dt, dr, Cs, eps_p, pars_int):
    """
    Vectorized implementation of solver_BE_simple using also
    a sparse (tridiagonal) matrix for efficiency.
...
#     for i in range(0,Nr+1):
    ...
    while erCp > err*np.max(Cp_n) or erTheta > err*np.max(thetaIntIn[num]):
        ...
        b2[-1] = b2[-1] + 2*beta*dr*k_ent*Cs[num]  # boundary conditions
        diags[:] = 1 + 2*beta + k1*(1-thetaInt_1)
        diags[-1] = 1 + 2*beta + k1*(1-thetaInt_1[0]) + 2*beta*dr*k_ext
...
#         print('Solving 1 eqation ', t3)
        ...
        while erv > er*np.max(v0) or eru > er*np.max(u0):
            arrCp0[:] = arrCpn 
            v0[Nx[1]:Nx[2]] = (b21 * u0[Nx[1]:Nx[2]] + v_n[Nx[1]:Nx[2]] + dt * k_ext * arrCp0[:,-1] / Ac) /\
...
#             print(max(v0))
            ...
            if k_ext != 0 and k_ent != 0 and Dp != 0:
                ...
                while ervp > er*np.max(v0) or erCp > er*np.max(arrCp0):
                    ...
                    wrapMPD=partial(MicroPDiff_multiprocess, Cs = Ac * vp0[Nx[1]:Nx[2]], CpIn = arrCpn, 
                                    thetaIntIn = thetaInt_n, R = R, ASpore = ASpore, 
                                    dt = dt, dr=dr, eps_p = eps_p, pars_int = pars_int)
#                     i = np.linspace(0, Nx[2]-Nx[1]-1, Nx[2]-Nx[1]).astype(int)
                    p = range(Nx[2]-Nx[1])
                    arry0 = Parallel(n_jobs=num_cores)(delayed(wrapMPD)(p) for p in range(Nx[2]-Nx[1]))
                    arry0 = np.transpose(np.array(arry0), (1, 0, 2))
                    arrCp0, thetaInt0 = arry0[0], arry0[1]
#                 print(np.min(abs(KH*u0[Nx[1]:Nx[2]])/(1+KH)))
#                     arrCp0[:], thetaInt0[:] = MicroPDiff_multiprocess(Cs = Ac * vp0[Nx[1]:Nx[2]], CpIn = arrCpn, 
#                                                          thetaIntIn = thetaInt_n, R = R, ASpore = ASpore, 
#                                                          dt = dt, dr=dr, eps_p = eps_p, pars_int = pars_int)
                    
                    
                    vp1[Nx[1]:Nx[2]] = (b21 * u0[Nx[1]:Nx[2]] + v_n[Nx[1]:Nx[2]] + dt * k_ext * arrCp0[:,-1] / Ac) /\
                            (1 + b22 + b21 * u0[Nx[1]:Nx[2]] + dt * k_ent)
            
            
                    wrapMPD=partial(MicroPDiff_multiprocess, Cs = Ac * vp1[Nx[1]:Nx[2]], CpIn = arrCpn, 
                                    thetaIntIn = thetaInt_n, R = R, ASpore = ASpore, 
                                    dt = dt, dr=dr, eps_p = eps_p, pars_int = pars_int)
                
                    arry1 = Parallel(n_jobs=num_cores)(delayed(wrapMPD)(p) for p in range(Nx[2]-Nx[1]))
                    arry1 = np.transpose(np.array(arry1), (1, 0, 2))
                    arrCp1, thetaInt1 = arry1[0], arry1[1]
        
#                     arrCp1[:], thetaInt1[:] = MicroPDiff_multiprocess(Cs = Ac * vp1[Nx[1]:Nx[2]], CpIn = arrCpn, 
#                                                          thetaIntIn = thetaInt_n, R = R, ASpore = ASpore, 
#                                                          dt = dt, dr=dr, eps_p = eps_p, pars_int = pars_int)
...
#            time_flow_res = time_fl2 - time_fl1
            ...
            v1[Nx[1]:Nx[2]] = (b21 * u1[Nx[1]:Nx[2]] + v_n[Nx[1]:Nx[2]] + dt * k_ext * arrCp0[:,-1] / Ac) /\
