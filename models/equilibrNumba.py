...
def MicroPDiff(Cs, CpIn, Dp, R, dt, dr,  eps_p):
    """
    ...
    diagonal = np.zeros((Nx, Nr+1))
    lower    = np.zeros((Nx, Nr))
    upper    = np.zeros((Nx, Nr))
    b2       = np.zeros((Nx, Nr+1))
    ...
    for i in nb.prange(Nx):
        Cp[i,:] = np.linalg.solve(A, b2[i])

    return Cp

@nb.jit(parallel = True)
def MicroPDiff1(CpIn, Dp, R, dt, dr, Cs, eps_p):
    """
    Vectorized implementation of Backward Euler scheme FOR 1 TIMESTEP using also
    a sparse (tridiagonal) matrix for efficiency. The function is used to short
    the text of iteration cycle. This version of solver uses scipy sparce matrix to solve SLE. 
    ===== ==========================================================
    Name  Description
    ===== ==========================================================
    Nr       Mesh resolution on the pore-length scale; mesh points are numbered
    ...
    for i in nb.prange(Nx):
        Cp[i,:] = scipy.sparse.linalg.spsolve(A, b2[i])
    
    return Cp
# Function that will be ran in parallel. Difference is using guvectorize instead of automatic jit function.  
@nb.guvectorize([(nb.float64[:,:],nb.float64[:,:],nb.float64[:,:])],'(m, m),(n,m)->(n,m)')
def solve_PDEs(A, b2, Cp):
    for i in range(Cp.shape[0]):
        Cp[i]=np.linalg.solve(A, b2[i])


def MicroPDiff3(Cs, CpIn, Dp, R, dt, dr,  eps_p):
    """
    Vectorized implementation of Backward Euler scheme FOR 1 TIMESTEP using also
    a sparse (tridiagonal) matrix for efficiency. The function is used to short
    the text of iteration cycle. Uses vectorized version of numbu in combination with numpy linalg solver of SLE. 
    ===== ==========================================================
    Name  Description
    ===== ==========================================================
    Nr       Mesh resolution on the pore-length scale; mesh points are numbered
...
def solver(CI, dt, f, T, CpI, opts, pars, er=0.000001,
           user_action=None, pore_plot=False):
    """
    ...
    *mass/Area/Length[1]/(1-eps[1])                  #Internal surface concentration of active sites mol/m2                           
    ...
    u2     = zeros(Nx[-1]+1)   # solution array for gas concentration at t[n+1] next approximation k+2
    ...
    for i in range(0,Nx[-1]+1):
        u_n[i]  = CI(x[i])
    # Set initial condition for each PDE that solves transport inside micropores for every spatial point in catalyst zone
    for i in range(0, Nr+1):
        Cp_n[i] = CpI(r[i])
    arrCpn[:] = Cp_n
    Fl_n[:] = Dp*(-(Cp_n[-1]-Cp_n[-2])/dr)
    ...
    for n in range(0, Nt):
        ...
        if Dp != 0 and KH != 0 :
            ...
            while eru > er*np.max(u0) or erFl > er:
                arrCp0[:] = MicroPDiff1(Cs = KH*u0[Nx[1]:Nx[2]], CpIn = arrCpn, Dp = Dp, R = R, dt = dt, 
                                                   dr=dr, eps_p = eps_p)
                ...
                arrCp1[:] = MicroPDiff1(Cs = KH*u1[Nx[1]:Nx[2]], CpIn = arrCpn, Dp = Dp, R = R, dt = dt, 
                                                   dr=dr, eps_p = eps_p)
