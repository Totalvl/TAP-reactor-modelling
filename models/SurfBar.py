...
#     for i in range(0,Nr+1):
    ...
    b2[:] = Cp_n
    b2[:, -1] = b2[:, -1] + 2* dt * Ac * k_ent /eps_p / dr * Cs[:]  # boundary conditions
...
def MicroPDiff_linalg(CpIn, Dp, dt, dr, Cs, k_ent, k_ext, opts):
    """
    Vectorized implementation of solver_BE_simple using also
    a sparse (tridiagonal) matrix for efficiency.
    """
...
#     Ap = np.diag(diagonal) + np.diag(lower, -1) + np.diag(upper, 1)
    Ap[rng, rng] = pore_diagonal[:]
    Ap[rng[1:], rng[1:]-1] = pore_lower[:]
    Ap[rng[:-1], rng[:-1]+1] = pore_upper[:]
...
#                 print(np.min(abs(KH*u0[Nx[1]:Nx[2]])/(1+KH)))
                    arrCp0[:] = MicroPDiff_linalg(Cs = vp0[Nx[1]:Nx[2]], CpIn = arrCpn, Dp = Dp, dt = dt,
                                                   dr=dr, k_ent = k_ent, k_ext = k_ext, opts = opts)

                    vp1[Nx[1]:Nx[2]] = (v_n[Nx[1]:Nx[2]] + b21 * u0[Nx[1]:Nx[2]] + dt * k_ext * arrCp0[:,-1] *eps_p / Sv / Ac) /\
                            (1 + b22 + b21 * u0[Nx[1]:Nx[2]] + dt * k_ent)
                    arrCp1[:] = MicroPDiff_linalg(Cs = vp1[Nx[1]:Nx[2]], CpIn = arrCpn, Dp = Dp, dt = dt,
                                                       dr=dr, k_ent = k_ent, k_ext = k_ext, opts = opts)    
...
#            time_flow_res = time_fl2 - time_fl1
            ...
            v1[Nx[1]:Nx[2]] = (v_n[Nx[1]:Nx[2]] + b21 * u1[Nx[1]:Nx[2]] + dt * k_ext * arrCp0[:,-1] *eps_p / Sv / Ac) /\
...
#     if show_plot:
#         np.where(array == np.amax(array))[0]
